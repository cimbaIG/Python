...
def potential_function():
    ...
    b = 2;
    Q = 2*math.pi;
    r = sympy.sqrt((x - a)**2 + (y - b)**2)
    theta = sympy.atan2(y - b, x - a)
    return (Q/2*math.pi)*sympy.ln(r**0.5)
...
plt.title(r'Izvor u (2,2)')
plot_streamlines(ax, u, v, (-1.99, 5.99), (-1.99, 5.97))

tz1 = plt.Circle((2,2), radius=0.03, facecolor='black', edgecolor='black')
ax.add_patch(tz1)
...
plt.savefig('zad_2_4_streamlines.pdf', bbox_inches='tight')
#Show figure
plt.show()
